...
import {connectDB, consulta, datoResponseReserva, dbEventos, respuestaReservaBorrar, usuarioAuth} from "./data";
...
describe('ReservaAppService', () => {
    ...
    it('Reserva consultar', async () => {
        ...
        const response = await application.inject({
            ...
            url: `${consulta}/reserva/4`,
        });
        const bodyResponse = JSON.parse(response.body);
        expect(response.statusCode).toBe(200);
        expect(bodyResponse).toEqual(datoResponseReserva);
    });

    it('Reserva borrar', async () => {
        (validarJWT as jest.Mock).mockReturnValue({
            id: usuarioAuth.id,
            usuario: usuarioAuth.usuario,
            correo: usuarioAuth.correo,
        ...
        const bodyResponse = JSON.parse(response.body);
        expect(response.statusCode).toBe(200);
        expect(bodyResponse).toEqual(respuestaReservaBorrar);
    });
});
